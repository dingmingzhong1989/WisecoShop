...
public class DataConstruct {
     ...
     * @param eventAction
     */
    public static void initEvent(StaticsListener eventInterface, String useID, String eventAction, String tagid, String comment){
        event = new Event();
        event.setTarget_id(tagid);
        event.set_user_id(useID);
        event.set_comment(comment);
        event.setAction_id(eventAction);
        event.setOperation_time(DateUtil.getDateString(System.currentTimeMillis(), "yyyy-MM-dd HH:mm:ss"));
